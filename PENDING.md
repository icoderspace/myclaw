...
- [ ] [2026-04-30] **PR Monitor 输出格式是否切纯 markdown** — 已发预览到 Hao DM，格式 OK；是否改 `pr-monitor/run.py` 让明天 09:00 BJT 生产 run 走纯 markdown（保留 `<at>` mention）等 Hao 拍板

## 已决 / 已闭环

- [x] [2026-04-30] Hao DM (1:1) conversationId 已拿到并记到 MEMORY.md，预览成功发送、Hao 确认收到
- [x] [2026-04-30] Hao 明确：以后有事主动往 webchat 推（他不一定及时刷）— 已记入 MEMORY.md
